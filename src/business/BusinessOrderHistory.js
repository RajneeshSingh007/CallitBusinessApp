...
export default class BusinessOrderHistory extends React.Component {
  ...
  textBelowOrder = (status, isDelivery) => {
    if (status === 1) return `התקבלה הזמנה חדשה ומחכה לאישור`;
    if (status === 2) return 'הזמנה בטיפול';
    if (status === 3 && isDelivery) return 'הזמנה מוכנה למשלוח';
    if (status === 3 && !isDelivery) return 'הזמנה מוכנה לאיסוף';
    if (status === 4) return 'הזמנה הסתיימה';
    if (status === -2) return 'הזמנה בוטלה על ידי לקוח';
          ...
          {this.state.viewid === 1 ? (
            <View
              ...
              <Button
                ...
                <Subtitle style={{color: 'white'}}>{'סינון'}</Subtitle>
              </Button>
              <DummyLoader
