...
export default class BranchDetailEdit extends React.Component {
  ...
  updateTime = (
    day,
    time, //what day to update + time given
  ) => {
    if (day === 1) {
  ...
  getTime = (
    day, // this is for displaying time inside this component
  ) => {
    let times;
    switch (day) {
      case 0:
            ...
            <View>
              <View
                ...
                <View
                  ...
                  <Checkbox.Android
                    ...
                    status={
                      this.state.hasDelivery === 0 ? 'unchecked' : 'checked'
                    }
                    onPress={() =>
                      this.setState({
                        hasDelivery: this.state.hasDelivery === 0 ? 1 : 0,
                      })
                    }
                    style={{
                      alignSelf: 'center',
              ...
              {this.state.showWeekDays ? (
                <View>
                  <TouchableWithoutFeedback
                    onPress={() =>
                      this.handleOnClickDayTimes(1, this.state.sun)
                    }>
                  ...
                  <TouchableWithoutFeedback
                    onPress={() =>
                      this.handleOnClickDayTimes(2, this.state.mon)
                    }>
                    <View style={styles.containerDayTime}>
                      <Subtitle style={styles.textDayTime}>
                        {this.getTime(1)}
                      </Subtitle>
                    </View>
                  </TouchableWithoutFeedback>
                  <TouchableWithoutFeedback
                    onPress={() =>
                      this.handleOnClickDayTimes(3, this.state.tue)
                    }>
                    <View style={styles.containerDayTime}>
                      <Subtitle style={styles.textDayTime}>
                        {this.getTime(2)}
                  ...
                  <TouchableWithoutFeedback
                    onPress={() =>
                      this.handleOnClickDayTimes(4, this.state.wed)
                    }>
                    <View style={styles.containerDayTime}>
                      <Subtitle style={styles.textDayTime}>
                        {this.getTime(3)}
                  ...
                  <TouchableWithoutFeedback
                    onPress={() =>
                      this.handleOnClickDayTimes(5, this.state.thur)
                    }>
                    <View style={styles.containerDayTime}>
                      <Subtitle style={styles.textDayTime}>
                        {this.getTime(4)}
                  ...
                  <TouchableWithoutFeedback
                    onPress={() =>
                      this.handleOnClickDayTimes(6, this.state.fri)
                    }>
                    <View style={styles.containerDayTime}>
                      <Subtitle style={styles.textDayTime}>
                        {this.getTime(5)}
                  ...
                  <TouchableWithoutFeedback
                    onPress={() =>
                      this.handleOnClickDayTimes(7, this.state.sat)
                    }>
                    <View style={styles.containerDayTime}>
                      <Subtitle style={styles.textDayTime}>
                        {this.getTime(7)}
              ...
              {/* //BusinessDelivery */}
              <TouchableWithoutFeedback
                onPress={() => NavigationAction.navigate('BusinessDelivery')}>
                <View
                  style={{
              ...
              <Modal
                ...
                <View style={{backgroundColor: 'white'}}>
                  ...
                  <Button
                    ...
                    <Subtitle style={{color: 'white'}}>{'סיום'}</Subtitle>
                  </Button>
                </View>
              </Modal>
            </Portal>
  ...
  renderSingleTimeBox(item, index) {
    return (
      <View style={styles.containerListHours}>
        <View
          style={{flexDirection: 'column', flex: 1, marginEnd: sizeWidth(2)}}>
  ...
  getTimeForList = (
    index,
    opening, //opening = true or false closing
  ) => {
  ...
  onSubmitTime = () => {
    ...
    if (!this.checkTimesCrossSD(times)) {
      Alert.alert(`שגיאה!`, 'לא ניתן לשים שעות חופפות', [{text: 'אישור'}], {
        cancelable: false,
      });
      return;
    }
  ...
  checkTimesCrossSD(
    ttimes, // //in time[1]:  `HH:mm-HH:mm` '15:00-20:00','15:00-20:00'
  ) {
    let opening;
    let closing;
    for (let i = 0; i < ttimes.length; i++) {
      if (i === 0) {
        // opening and closing are empty so we assign them
        ...
        if (curOpening[0] > curClosing[0]) {
          //check the acitivity time itself its is correct 15:00-20:00 is good, 20:00-15:00 is bad
          return false;
        } else if (
          curOpening[0] === curClosing[0] &&
          curOpening[1] > curClosing[1]
        ) {
          // this is if Hours and the same check mins
          return false;
        }
